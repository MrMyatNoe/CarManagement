...
export class HttpConfigInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    if (!req.urlWithParams) return EMPTY;
    return next.handle(req).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        console.log("HTTP ERROR", error.error.status);
        if (error.error) {
          ...
          this.toastrService.error(
            error.error.message || "Sorry!, Try again later"
          );

        if (error.error.status === 190) {
          console.log("error event", error.error.status);
          this._auth.logout();
